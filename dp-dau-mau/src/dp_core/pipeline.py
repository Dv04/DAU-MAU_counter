...
def _seed_for(metric: str, day: dt.date, default_seed: int) -> int:
    digest = sha256(f"{metric}:{day.isoformat()}:{default_seed}".encode("utf-8")).digest()
...
class PipelineManager:
    def __init__(
        self,
        config: Optional[config_module.AppConfig] = None,
    ...
    def ingest_event(self, event: EventRecord) -> None:
        ...
        if event.op == "-":
            ...
            if day_str not in days:
                days.append(day_str)
            erasure_entry = ErasureEntry(erasure_id=None, user_root=user_root, days=days, pending=True)
    ...
    def get_mau_release(self, end_day: dt.date, window_days: Optional[int] = None) -> dict[str, Any]:
        self.replay_deletions()
        window = window_days or self.config.sketch.mau_window_days
        end_day_str = end_day.isoformat()
        value, _union = self.window_manager.get_mau(
            end_day_str, window, self.events_loader
        )
